...
void HKStdAuth::Auth1_keys_generator(uint8_t *persistentKey, uint8_t *volatileKey)
{
  ...
  LOG(D, "Persistent Key: %s", fmt::format("{:02X}", fmt::join(std::span<uint8_t>(persistentKey, 32), "")).c_str());
  LOG(D, "Volatile Key: %s", fmt::format("{:02X}", fmt::join(std::span<uint8_t>(volatileKey, 48), "")).c_str());
}
...
void HKStdAuth::Auth1_keying_material(uint8_t *keyingMaterial, const char *context, uint8_t *out, size_t outLen)
{
  ...
  LOG(D, "DATA Material Length: %d, Data: %s", sizeof(dataMaterial), fmt::format("{:02X}", fmt::join(dataMaterial, "")).c_str());
