 ...
 */
HKAuthenticationContext::HKAuthenticationContext(PN532 &nfc, readerData_t &readerData, nvs_handle &savedData) : readerData(readerData), savedData(savedData), nfc(nfc), transactionIdentifier(16)
{
    ...
    if(foundEndpoint == nullptr){
      ...
      if(std::get<1>(stdAuth) != nullptr){
        ...
        if ((flowUsed = std::get<4>(stdAuth)) == kFlowSTANDARD)
        {
          ...
          LOG(V, "New Persistent Key: %s", utils::bufToHexString(foundEndpoint->endpoint_prst_k.data(), foundEndpoint->endpoint_prst_k.size()).c_str());
        }
      }
      if (std::get<4>(stdAuth) == kFlowNext || hkFlow == kFlowATTESTATION) {
        ...
        if ((flowUsed = std::get<2>(attestation)) == kFlowATTESTATION) {
          ...
          LOG(V, "New Persistent Key: %s", utils::bufToHexString(endpoint.endpoint_prst_k.data(), endpoint.endpoint_prst_k.size()).c_str());
